...
Windows 本地预览要求 Windows 10 build 19041+ 或 Windows 11 x64、已开启 CPU 虚拟化，并使用 Administrators 用户和 UAC。推荐 4 核 CPU、8 GB 内存、50 GB 可用磁盘；2 核、4 GB 内存、10 GB 可用磁盘是预检最低门槛，低于推荐配置时安装器会提示风险但继续尝试。安装器会创建独立的 Rainbond WSL2 环境，首次下载可能较久且可能重启一次；下载会显示进度，重启后从已保存断点继续。Windows 登录后通过 `http://127.0.0.1:7070` 访问 Console。Windows 10/11 真机验收完成前不视为正式支持。
...
- 检查系统、架构、推荐资源（4 核 CPU、8 GB 内存、50 GB 磁盘）、最低资源门槛（2 核 CPU、4 GB 内存、10 GB 磁盘）、权限和端口；低于推荐配置时只提示风险
