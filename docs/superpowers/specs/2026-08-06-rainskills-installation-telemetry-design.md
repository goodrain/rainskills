...
除 `reason`、`blocked_reason` 外，字段均为固定白名单；未适用字段显式使用 `null`，禁止把任意命令输出塞入 `metadata`。错误代码、阶段、步骤、动作、传输方式使用稳定的小写枚举。`reason` 最多 1024 个 UTF-8 字节，先脱敏后截断；`blocked_reason` 和 `interrupt_signal` 只允许固定枚举。
...
每次请求发送单个 JSON 事件，事件包含 `schema`、`event_id`、`install_attempt_id`、阶段字段以及现有兼容摘要字段。服务端以 `event_id` 建立唯一约束或幂等键：同一事件重复 POST 必须返回 2xx 且只计数一次。客户端不能依赖响应正文；旧服务若忽略未知字段，仍必须按现有 `phase/status/failure_stage/failure_category` 字段解析四类摘要。发布前使用固定 JSON golden 测试验证新旧字段同时存在、敏感字段不出现和重复发送不改变计数语义。
...
`control_distro`、SSH 主机和 Console 地址不写入事件；仅记录 `transport`、认证方式类别和目标枚举。Windows action 名称进入 `action` 白名单，WSL 发行版版本只记录受限版本号事实，不记录路径或命令输出。
...
本地落盘和远程上传使用同一套 allowlist 和过滤器：过滤 JWT、Bearer/Basic 认证头、密码/Token/Secret/API Key 赋值、`--token`/`--password` 参数值、URL userinfo/query/fragment、SSH 用户名/主机、PowerShell/WSL 输出和原始命令行。过滤后文本最多 1024 个 UTF-8 字节；只保留固定的错误代码、阶段、退出码、HTTP 状态码和可重试标记。`eid`、旧安装摘要字段仍可按现有接口契约发送，但不得把它们拼接进 URL 或日志文本。
