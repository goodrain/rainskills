...
8. Before creating any image-backed component, evaluate the `image` value for a public-registry pull and, unless the user already opted out earlier in the same run, ask once whether to keep the raw image or switch to a registry mirror. This **includes bare Docker Hub references** such as `nginx:latest`, `library/nginx:latest`, or `mysql/mysql-server:8.0` — they implicitly resolve to `docker.io/library/...` or `docker.io/...` and must be treated the same as explicit `docker.io/...`. Other in-scope registries: `quay.io/...`, `gcr.io/...`, `ghcr.io/...`, `k8s.gcr.io/...`, `registry.k8s.io/...`. Skip only when the image is already on a known mirror (`docker.1ms.run/...`, `m.daocloud.io/...`, etc.) or on a private registry. Recommend `docker.1ms.run/<full-path>` first (e.g. `nginx:latest` → `docker.1ms.run/library/nginx:latest`, `docker.io/library/postgres:17` → `docker.1ms.run/library/postgres:17`); reuse any mirror already chosen for another in-scope component instead of introducing a second one.
