...
- **Windows 下载、镜像仓库或摘要失败**：检查代理是否允许策略文件列出的 HTTPS 来源，包括 Rainbond 官方安装使用的 `registry.cn-hangzhou.aliyuncs.com`。Ubuntu 日期镜像使用固定文件大小和固定摘要；HTTP 响应总大小、断点续传 `Content-Range`、实际响应流和最终文件都必须匹配该版本。当前源异常时会立即停止写入、隔离缓存并切换下一个固定镜像，全部来源失败后才输出最终长度和摘要；不得跳过校验。
