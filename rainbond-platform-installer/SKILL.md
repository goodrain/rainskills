...
- 用户只在自己电脑的系统终端执行消息中的精确 `ssh prepare` 命令。该命令只生成/复用默认 ED25519 公钥并准备免密连接，不安装 Rainbond，也不修改 onboarding、运行环境或授权状态。
- Agent 不得自行运行 `ssh-keyscan`、`ssh-copy-id` 或修改 `known_hosts`；只能让用户执行 helper 给出的版本锁定 `ssh prepare` argv。
...
6. 如果 helper 输出 `platform.ssh-authentication`，只原样展示该正文并等待用户回复“已完成”；随后在原任务中重跑同一安装 argv。否则保持操作附着并让 helper 下载、安装和验证，不得只凭官方脚本退出码推断成功。
...
主机集群第一次进入 configuration 阶段时，必须自动生成受保护的三节点 `cluster.yaml` 示例文件并停止等待。只告诉用户文件位置，以及“一次性修改服务器地址、SSH 端口和节点角色，完成后回复‘已完成’”；不得逐台询问节点字段。生成模板禁止 password、private key、Token 或 secret 等敏感字段，SSH 密码只能由系统 `ssh` 在终端中读取。

用户继续同一 onboarding 时，读取同一份受保护文件并一次性列出全部可确定的 YAML、节点、角色、bootstrap、etcd 和存储问题。存在问题时不得发起 SSH、下载或安装；配置通过后展示节点数、etcd 数量、bootstrap 和存储模式，直接进入并发 SSH 预检。已有 `--cluster-config` 高级导入仍保留原始字节和未知字段。
...
- 指纹确认和一次 SSH 密码只允许发生在用户主动执行的 `ssh prepare` 进程中，由系统 `ssh` 直接读取。成功后所有预检、安装、传输和验收均使用 `BatchMode=yes`，不得再次请求密码。
- `ssh prepare` 完成后不得在系统终端继续安装或授权。浏览器授权仅在原 AI 任务的平台验收成功后发生一次；授权完成后用同一 `onboarding-id` 自动恢复原始应用操作。
